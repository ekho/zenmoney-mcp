...
Меняем только общую read-only логику планирования, MCP-схемы, dispatch, прямых
потребителей изменённого контракта и тесты. Не сохраняем финансовые overrides,
не меняем сущности ZenMoney и синхронизацию, не расширяем payoff planner и пока
не добавляем `get_financial_position`.

Основная реализация остаётся в `planning.py` и `server.py`. В
`decision/debt.py` и `decision/scenarios.py` меняется только чтение нового
breaking-контракта; поведение payoff planner не расширяется. Новый
domain-модуль, таблица БД, миграция или зависимость в этой фазе не дают пользы,
но добавляют код и владение.
