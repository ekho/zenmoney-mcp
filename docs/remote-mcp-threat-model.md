...
| Sync control state | Separate named volume, writable only by MCP and the sync worker; strict, bounded JSON with no credentials or financial data. |
| MCP outputs | Financial analytics are read-only and bounded. `force_sync` can request a cache refresh but cannot access credentials or write ZenMoney. |
...
| Malicious MCP input | The SDK owns protocol parsing; remote dispatch rejects unknown and excluded tools. Schemas are strict. `force_sync` is truthfully non-read-only/open-world; every other remote tool remains read-only/closed-world. |
...
| Container compromise | All roles use read-only roots, dropped capabilities, `no-new-privileges`, and `/tmp` tmpfs; Python roles run as UID/GID `10001`. A compromised MCP still cannot write the financial cache or read the ZenMoney credential, but it can exhaust or corrupt the writable control volume and deny forced synchronization. The full tunnel image is not claimed to be non-root. Keep pinned images current and the host patched. |
